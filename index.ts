...
app.post("/identify", async (req: Request, res: Response) => {
  let { email, phoneNumber }: IdentifyRequest = req.body;
  ...
  try {
    ...
    if (initialContacts.length === 0) {
      ...
      return res.status(200).json({
        contact: {
          ...
          phoneNumbers: [primaryContact.phoneNumber].filter(
    ...
    const allContacts = Array.from(
      new Set([...initialContacts, ...additionalContacts])
    );
    ...
    const consolidatedContact: ConsolidatedContact = {
      ...
      secondaryContactIds: Array.from(
        new Set(secondaryContacts.map((contact) => contact.id))
      ),
    };
